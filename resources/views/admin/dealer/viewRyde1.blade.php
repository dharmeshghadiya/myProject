...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">{{ $company_details->name }} {{ config('languageString.rydes') }}</h4>
            </div>
        </div>
        <div class="d-flex my-xl-auto right-content">
            <div class="pr-1 mb-3 mb-xl-0">

                <a href="{{ url('admin/ryde/create/'.$company_details->id.'/'.$branch_id) }}" class="btn btn-info  mr-2">
                        ...
                        <table class="table mg-b-0 text-md-nowrap" id="data-table">
                            ...
                            @foreach($vehicles as $vehicle)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ url('admin/ryde/edit/'.$vehicle->company_address_id.'/'.$vehicle->id) }}" class="btn btn-sm btn-outline-info waves-effect waves-light" data-toggle="tooltip" data-placement="top" title="{{config('languageString.edit')}}"><i class="bx bx-pencil font-size-16 align-middle"></i></a>
                                        <button class="vehicle-details btn btn-outline-info btn-sm" data-id="{{ $vehicle->id }}"
                                                data-toggle="tooltip" data-placement="top"
                                                title="{{ config('languageString.view_details') }}">
                                            <i class="bx bx-bullseye font-size-16 align-middle"></i>
                                        </button>
                                        @if($vehicle->status == 'Active')
                                        <button data-id="{{$vehicle->id}}" data-status="{{'InActive'}}" class="status-change btn btn-sm btn-outline-success waves-effect waves-light" data-effect="effect-fall" data-toggle="tooltip" data-placement="top" title="{{config('languageString.in_active')}}" ><i class="bx bx-refresh font-size-16 align-middle"></i></button>
                                        @else
                                        <button data-id="{{$vehicle->id}}" data-status="{{'Active'}}" class="status-change btn btn-sm btn-outline-success waves-effect waves-light" data-effect="effect-fall" data-toggle="tooltip" data-placement="top" title="{{config('languageString.active')}}" ><i class="bx bx-refresh font-size-16 align-middle"></i></button>
                                        @endif
